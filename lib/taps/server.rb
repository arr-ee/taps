...
class Server < Sinatra::Base
  ...
  error do
    ...
    if e.kind_of?(Taps::BaseError)
      ...
      halt 412, MultiJson.encode({ 'error_class' => e.class.to_s, 'error_message' => e.message, 'error_backtrace' => e.backtrace.join("\n") })
    else
      "Taps Server Error: #{e}\n#{e.backtrace}"
    end
  end
  ...
  post '/sessions/:key/push/verify_stream' do
    ...
    content_type 'application/json'
    MultiJson.encode({ :state => stream.to_hash })
  end
  ...
  post '/sessions/:key/pull/table' do
    ...
    session.conn do |db|
      state = MultiJson.decode(params[:state]).symbolize_keys
      stream = Taps::DataStream.factory(db, state)
      encoded_data = stream.fetch.first
    end

    checksum = Taps::Utils.checksum(encoded_data).to_s
    json = MultiJson.encode({ :checksum => checksum, :state => stream.to_hash })

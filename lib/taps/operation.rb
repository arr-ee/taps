...
class Operation
  ...
  def store_session
    ...
    File.open(file, 'w') do |f|
      f.write(MultiJson.encode(to_hash))
    end
  end
...
class Pull < Operation
  ...
  def fetch_remote_tables_info
    ...
    max_retries = 10
    begin
      tables = MultiJson.decode(session_resource['pull/table_names'].get(http_headers).to_s)
  ...
  def pull_indexes
    ...
    idxs = MultiJson.decode(session_resource['pull/indexes'].get(http_headers).to_s)
...
class Push < Operation
  ...
  def push_indexes
    idxs = MultiJson.decode(Taps::Utils.schema_bin(:indexes_individual, database_url))
